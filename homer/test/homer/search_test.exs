defmodule Homer.SearchTest do
  ...
  describe "offer_requests" do
    ...
    @invalid_attrs %{allowed_airlines: nil, departure_date: nil, destination: nil, origin: nil, sort_by: nil}
    ...
    test "create_offer_request/1 with valid data creates a offer_request" do
      valid_attrs = %{allowed_airlines: [], departure_date: ~D[2021-12-11], destination: "some destination", origin: "some origin", sort_by: "some sort_by"}

      assert {:ok, %OfferRequest{} = offer_request} = Search.create_offer_request(valid_attrs)
      assert offer_request.allowed_airlines == []
      assert offer_request.departure_date == ~D[2021-12-11]
    ...
    test "update_offer_request/2 with valid data updates the offer_request" do
      offer_request = offer_request_fixture()
      update_attrs = %{allowed_airlines: [], departure_date: ~D[2021-12-12], destination: "some updated destination", origin: "some updated origin", sort_by: "some updated sort_by"}

      assert {:ok, %OfferRequest{} = offer_request} = Search.update_offer_request(offer_request, update_attrs)
      assert offer_request.allowed_airlines == []
      assert offer_request.departure_date == ~D[2021-12-12]
    ...
    test "update_offer_request/2 with invalid data returns error changeset" do
      offer_request = offer_request_fixture()
      assert {:error, %Ecto.Changeset{}} = Search.update_offer_request(offer_request, @invalid_attrs)
